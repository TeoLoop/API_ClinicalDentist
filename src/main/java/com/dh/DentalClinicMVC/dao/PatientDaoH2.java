...
public class PatientDaoH2 implements IDao<Patient>{
    ...
    private final static String SQL_INSERT = "INSERT INTO PATIENTS (NAME, LASTNAME, EMAIL, CARD_IDENTITY, ADMISSION_OF_DATE, ADDRESS_ID )"+
            "VALUES (?,?,?,?,?,?)";

    private final static String SQL_SELECT_ID = "SELECT * FROM PATIENTS WHERE ID =?";

    private static final String SQL_UPDATE = "UPDATE PATIENTS SET NAME=?, LASTNAME=?, EMAIL=?, CARD_IDENTITY=?, ADMISSION_OF_DATE=?, ADDRESS_ID=? WHERE ID =?";
    ...
    public List<Patient> findAll() {
        ...
        try {
            ...
            while (rs.next()){
                ...
                patientList.add(new Patient(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4),
                        rs.getInt(5), rs.getDate(6).toLocalDate(), address));
            }
